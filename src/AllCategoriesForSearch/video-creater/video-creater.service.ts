...
@Injectable()
export class VideoCreaterService {
  private readonly logger = new Logger(AppService.name);
  private id: string | number;

  constructor(
    @InjectRepository(VideoCreaterEntity)
    private repository: Repository<VideoCreaterEntity>,
    private readonly httpService: HttpService,
    private logsService: LogsService,
    private redisService: RedisService,
  ) {
    this.id = process.env['ID_CHAT_VIDEO_CRATER']; // 7й
  }
  ...
  async sendPostToTelegram(item, tokenBot, telegramLimiter) {
    try {
      const chatId = process.env['CHAT_VIDEO_CRATER'];
